...
// lavoro. Sotto la tabella c'e' l'elenco delle diciture che nessuna
// regola ha saputo classificare, e da li' le agganci a mano.
//
// LA CASCATA (migrazioni 34 / 35 / 35b), in ordine. Vince il primo
// livello che risponde, e ogni gruppo dichiara da quale livello e' nato:
//   0 forzato sulla riga — una marcatura sulla singola riga, vince su tutto
//   1 agganciato a mano  — una regola esatta del dizionario, decisa da te
//   2 modello            — una regola a modello del dizionario
//   3 categoria          — quando la dicitura e' vuota (gli alloggi)
//   4 prefisso del menu  — quello che sta prima del trattino
//   5 categoria (rete)   — la rete di sicurezza, con un nome suo (35b)
//   6 da classificare    — resta visibile, non sparisce mai
//
// I MACRO-GRUPPI (migrazione 37) sono uno strato SOPRA la cascata, non
// un livello dentro di essa: la tabella hic_macro_gruppi traduce il nome
// del gruppo in un macro-gruppo, e hic_consumi_macro somma il risultato
// di hic_consumi. La cascata non viene toccata, quindi questa scheda e
// il pannello delle diciture non possono contraddirsi.
//
...
//
// ATTENZIONE alla colonna Prenotazioni: c'e' solo nella vista per
// gruppi. E' un conteggio di prenotazioni distinte calcolato DENTRO
// ogni gruppo, quindi non e' sommabile: una cena con antipasto, primo e
// vino conterebbe tre volte. Per questo nella vista per macro-gruppi al
// suo posto c'e' il numero di gruppi che confluiscono nel macro.
// ============================================================

function classeOrigine(origine) {
  if (origine === 'forzato sulla riga') return 'text-xs px-2 py-0.5 rounded-full bg-wine-50 text-wine-900 border border-wine-300';
  ...
  if (origine === 'categoria (rete)') return 'text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-800 border border-green-300';
...
function classeInterruttore(attivo) {
  if (attivo) return 'px-3 py-1.5 rounded-lg text-sm font-medium border bg-wine-800 text-white border-wine-800';
  return 'px-3 py-1.5 rounded-lg text-sm font-medium border bg-white text-gray-700 border-gray-300 hover:border-wine-400 hover:text-wine-800';
}

// ------------------------------------------------------------
// Tabella per MACRO-GRUPPI
// ------------------------------------------------------------
function TabellaMacro(props) {
  var righe = props.righe;

  var lordoTotale = 0;
  var righeZeroTotali = 0;
  var gruppiTotali = 0;
  for (var i = 0; i < righe.length; i++) {
    lordoTotale = lordoTotale + Number(righe[i].lordo);
    righeZeroTotali = righeZeroTotali + Number(righe[i].righe_zero);
    gruppiTotali = gruppiTotali + Number(righe[i].gruppi);
  }

  if (righe.length === 0) {
    return <div className="text-sm text-gray-400 py-6 text-center">Nessun addebito nel periodo scelto.</div>;
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs uppercase tracking-wider text-gray-400 border-b border-gray-200">
            <th className="py-2 pr-3 font-semibold">Macro-gruppo</th>
            <th className="py-2 px-3 font-semibold text-right">Gruppi</th>
        ...
        <tbody>
          {righe.map(function(riga) {
            ...
            var orfano = riga.macro === 'Senza macro-gruppo';
            return (
              <tr key={riga.macro} className="border-b border-gray-100">
                <td className="py-2 pr-3">
                  <span className={orfano ? 'text-amber-800 font-semibold' : 'text-gray-800'}>{riga.macro}</span>
                </td>
                <td className="py-2 px-3 text-right tabular-nums text-gray-800">{numero(riga.gruppi)}</td>
        ...
        <tfoot>
          <tr className="border-t-2 border-gray-200 font-semibold">
            ...
            <td className="py-2 px-3 text-right tabular-nums text-gray-700">{numero(gruppiTotali)}</td>
...
export default function SchedaConsumi(props) {
  ...
  var [vista, setVista] = useState('macro');
  var [macro, setMacro] = useState([]);
  ...
  useEffect(function() {
    ...
    // hic_consumi si carica SEMPRE, anche nella vista per macro: serve a
    // riempire i pulsanti dei gruppi esistenti dell'aggancio a mano.
    Promise.all([
      supabase.rpc('hic_consumi', parametri),
      supabase.rpc('hic_consumi_macro', parametri),
  ...
  var perMacro = vista === 'macro';

  var orfani = 0;
  for (var m = 0; m < macro.length; m++) {
    if (macro[m].macro === 'Senza macro-gruppo') orfani = Number(macro[m].gruppi);
  }
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-2 mb-1">
          <div className="text-sm font-semibold text-gray-800">
            {perMacro ? 'Consumi per macro-gruppo' : 'Consumi per gruppo'}
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={function() { setVista('macro'); }}
              className={classeInterruttore(perMacro)}>
              Per macro-gruppi
            </button>
            <button
              type="button"
              onClick={function() { setVista('gruppi'); }}
              className={classeInterruttore(!perMacro)}>
              Per gruppi
            </button>
          </div>
        ...
        {perMacro ? (
          <div className="text-xs text-gray-500 mb-3">
            Ogni riga somma i gruppi che le appartengono, secondo la tabella di traduzione
            hic_macro_gruppi. La colonna Prenotazioni non c&apos;e&apos;: e&apos; un conteggio calcolato
            dentro ogni gruppo e non si puo&apos; sommare, perche&apos; una cena con antipasto, primo e
            vino conterebbe tre volte. Al suo posto trovi quanti gruppi confluiscono nel macro.
          </div>
        ) : (
          <div className="text-xs text-gray-500 mb-3">
            Una riga per gruppo, con il livello della cascata da cui il gruppo e&apos; nato. La colonna
            Notti e&apos; la somma delle quantita&apos;, e in Hotel in Cloud la quantita&apos; di un
            addebito e&apos; il numero di notti, non il numero di pezzi. Il numero solido e&apos; la
            colonna Prenotazioni.
          </div>
        )}

        {perMacro
          ? <TabellaMacro righe={macro} />
          : <TabellaGruppi righe={gruppi} />}

        {perMacro && orfani > 0 && (
          <div className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-lg p-3 mt-3">
            {numero(orfani)} gruppi non hanno ancora un macro-gruppo e sono raccolti nella riga
            &quot;Senza macro-gruppo&quot;. Non spariscono e non tolgono un euro al totale: vanno abbinati
            nella tabella hic_macro_gruppi, senza bisogno di un aggiornamento del programma.
          </div>
        )}

        <div className="text-xs text-gray-400 mt-3">
          La colonna &quot;A zero&quot; conta le righe che valgono zero euro. Non sono quasi mai gift card:
          la riga a zero piu&apos; comune e&apos; una portata compresa in un menu a prezzo fisso. Il
          servizio c&apos;e&apos; stato e il denaro e&apos; entrato, ma su un&apos;altra riga della stessa
          prenotazione.
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="text-xs text-gray-500 mb-3">
          ...
          Questo pannello lavora sui gruppi, quindi non cambia con l&apos;interruttore qui sopra.
        </div>
