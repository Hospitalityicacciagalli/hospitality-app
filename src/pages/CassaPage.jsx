...
function FormMovimento(props) {
  ...
  var [metodoId, setMetodoId] = useState(isModifica ? movimentoEsistente.metodo_pagamento_id : ID_CONTANTI);
  var [importo, setImporto] = useState(isModifica ? String(movimentoEsistente.importo) : '');
  ...
  var [tagli, setTagli] = useState(taglioVuoto());
  var [saving, setSaving] = useState(false);
  var [errore, setErrore] = useState('');

  var tipoInfo = getTipoInfo(tipo);
  var isContanti = metodoId === ID_CONTANTI;
  ...
  function handleSave() {
    ...
    var movimento = {
      cassa_id: cassaId,
      data: oggiISO(),
      ora: new Date().toTimeString().slice(0, 8),
      tipo: tipo,
    ...
    var promise = isModifica
      ? supabase.from('movimenti_cassa').update(movimento).eq('id', movimentoEsistente.id).select()
      : supabase.from('movimenti_cassa').insert([movimento]).select();
    ...
    promise.then(function(result) {
      ...
      if (isContanti) salvato._tagli = Object.assign({}, tagli);
      onSave(salvato, isModifica);
    });
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
      <div style={{ background: '#1a1f2e', borderRadius: '16px', width: '100%', maxWidth: '540px', padding: '28px', boxShadow: '0 24px 64px rgba(0,0,0,0.5)', border: '1px solid #2d3448', maxHeight: '92vh', overflowY: 'auto' }}>
        ...
        <input type="text" placeholder="es. Direttore, Reception, Florestano..." value={provenienza} onChange={function(e) { setProvenienza(e.target.value); }} style={S.input} />
        ...
        {alertsVivi.length > 0 && (
          <div style={{ background: '#f59e0b10', border: '1px solid #f59e0b44', borderRadius: '8px', padding: '10px 14px', marginBottom: '16px' }}>
            ...
            <div style={{ color: '#78716c', fontSize: '12px', marginTop: '6px' }}>Puoi salvare comunque.</div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
          ...
          <button onClick={handleSave} disabled={saving} style={Object.assign({}, S.btnPrimario, { opacity: saving ? 0.6 : 1 })}>
            {saving ? 'Salvataggio...' : (isModifica ? 'Aggiorna' : 'Salva movimento')}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderRadius: '10px', background: coloreRiga, border: '1px solid ' + coloreBordo, opacity: m.annullato ? 0.5 : 1, position: 'relative' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {m._tagli && totaleTagli(m._tagli) > 0 && (
          <div style={{ marginTop: '4px', display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
            {TAGLI.map(function(t) {
              var q = m._tagli[taglioKey(t)] || 0;
              if (q === 0) return null;
      ...
      {!m.annullato && (
        <div style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', display: 'flex', gap: '4px' }}>
          <button onClick={function() { onModifica(m); }} title="Modifica"
            style={{ background: '#1e2538', border: '1px solid #2d3448', color: '#64748b', cursor: 'pointer', fontSize: '13px', padding: '5px 9px', borderRadius: '6px' }}
...
export default function CassaPage() {
  ...
  useEffect(function() {
    supabase
      ...
      .select('tipo, metodo_pagamento_id, importo')
      .eq('cassa_id', cassaId)
      .eq('annullato', false)
      .eq('metodo_pagamento_id', ID_CONTANTI)
      .lt('data', data)
      .then(function(r) {
  ...
  movimenti.forEach(function(m) {
    ...
    var isUscita = tipoInfo.segno === '-';
    var isContante = m.metodo_pagamento_id === ID_CONTANTI;
    if (!m.is_fattoria && isEntrata && (m.tipo === 'scontrino' || m.tipo === 'fattura' || m.tipo === 'caparra')) totFiscaleGiorno += m.importo;
    if (m.tipo === 'prelievo') totPrelieviGiorno += m.importo;
    if (isContante) {
      if (isEntrata) contantiGiorno += m.importo;
      if (isUscita) contantiGiorno -= m.importo;
    }
    if (m._tagli) {
  ...
  function handleAnnulla(id) {
    supabase.from('movimenti_cassa').update({ annullato: true, annullato_da: userId, annullato_at: new Date().toISOString() }).eq('id', id).then(function(r) {
      if (!r.error) {
        setMovimenti(function(prev) { return prev.map(function(m) { return m.id === id ? Object.assign({}, m, { annullato: true }) : m; }); });
      }
    });
  }

  function handleModifica(movimento) {
    setMovimentoDaModificare(movimento);
    setShowForm(true);
  }
      ...
      <div style={{ background: '#1a1f2e', borderBottom: '1px solid #2d3448', padding: '0 24px', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', marginLeft: 'auto' }}>
          {[{ id: 'movimenti', label: 'Movimenti' }, { id: 'chiusura', label: 'Chiusura' }, puoVedereCassaforte ? { id: 'cassaforte', label: 'Cassaforte' } : null].filter(Boolean).map(function(s) {
            ...
            return <button key={s.id} onClick={function() { setSezione(s.id); }} style={{ padding: '8px 16px', border: 'none', cursor: 'pointer', background: 'transparent', color: sel ? '#c9a96e' : '#64748b', fontWeight: sel ? '600' : '400', fontSize: '14px', borderBottom: sel ? '3px solid #c9a96e' : '3px solid transparent', transition: 'all 0.15s' }}>{s.label}</button>;
          })}
        </div>
      </div>
      ...
      <div style={{ padding: '24px', maxWidth: '960px', margin: '0 auto' }}>
        ...
        {sezione === 'movimenti' && (
          <div>
            ...
            <RiepilogoTagliCassa tagliCassa={tagliGiorno} />

            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <div style={{ color: '#64748b', fontSize: '14px' }}>
                {loading ? 'Caricamento...' : (movimenti.length === 0 ? 'Nessun movimento' : movimenti.length + ' movimenti')}
              </div>
              <button onClick={function() { setMovimentoDaModificare(null); setShowForm(true); }} style={{ background: '#c9a96e', color: '#0f1117', border: 'none', borderRadius: '8px', padding: '10px 20px', cursor: 'pointer', fontWeight: '700', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                + Nuovo movimento
              </button>
            </div>
